    ...
    when (node.type) {
        MarkdownElementTypes.MARKDOWN_FILE, MarkdownElementTypes.PARAGRAPH, MarkdownElementTypes.UNORDERED_LIST, MarkdownElementTypes.ORDERED_LIST -> {
            ...
            val children = node.children.filterIndexed { index, childNode ->
                !(childNode.type == MarkdownTokenTypes.EOL && index > 0 && headlineTypes.contains(
                    node.children[index - 1].type
                ))
            }
        ...
        MarkdownElementTypes.INLINE_LINK -> {
            val linkDestination =
                node.children.findLast { it.type == MarkdownElementTypes.LINK_DESTINATION }
                    ?: return this
            val linkText =
                node.children.find { it.type == MarkdownElementTypes.LINK_TEXT }?.children

            withAnnotation(UrlAnnotation(linkDestination.getTextInNode(markdownText).toString())) {
                withStyle(SpanStyle(linkColor)) {
                    linkText
                        ?.filterIndexed { index, _ -> index != 0 && index != linkText.size - 1 }
                        ?.forEach { childNode ->
                            appendMarkdown(
                                markdownText = markdownText,
                                node = childNode,
        ...
        MarkdownElementTypes.IMAGE -> {
            val linkNode = node.children.findLast { it.type == MarkdownElementTypes.INLINE_LINK }
                ?: return this
            val imageUrlNode =
        ...
        MarkdownElementTypes.ATX_3 -> {
            val content =
                node.children.find { it.type == MarkdownTokenTypes.ATX_CONTENT || it.type == MarkdownTokenTypes.SETEXT_CONTENT }
