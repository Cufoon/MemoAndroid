...
import cufoon.memo.android.ext.appendMarkdown
...
fun Markdown(
    ...
    BoxWithConstraints {
        val (annotatedString, inlineContent) = remember(text, maxWidth) {
            val markdownAst = MarkdownParser(GFMFlavourDescriptor()).parse(
            ...
            builder.appendMarkdown(
                ...
                onCheckbox = { key, startOffset, endOffset ->
                    inlineContent[key] = InlineTextContent(
                        ...
                        Checkbox(
                            checked = checkboxText.length > 1 && checkboxText[1] != ' ',
                            onCheckedChange = {
